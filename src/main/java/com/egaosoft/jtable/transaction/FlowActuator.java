...
public class FlowActuator {
    ...
    static {
        if (MY_PROP != null) {
            DRUID_PLUGIN = new DruidPlugin(MY_PROP.get("flow.jdbcUrl"), MY_PROP.get("flow.userName"),
                MY_PROP.get("flow.password"));
            ACTIVE_RECORD = new ActiveRecordPlugin("flowActiveRecord", DRUID_PLUGIN);
        }
    }
    ...
    public static TransactionFlow getFlow(Long flowId) {
        try {
            ...
            List<FlowNode> nodeList = FlowNode.dao
                .find("SELECT * FROM flow_node WHERE flowId = ? ORDER BY flow_node.index ASC", flow.getFlowId());
            destoryDataSource();
            List<TransactionNode> transactionNodeList = new ArrayList<TransactionNode>();
            for (FlowNode node : nodeList) {
                TransactionNode transactionNode = new TransactionNode();
    ...
    public static void start(TransactionFlow flow) {
        ...
        RpcContext.getContext().setAttachment(getNodeIndex(), "0");
        RpcContext.getContext().setAttachment(getNodeIndexMax(), String.valueOf(flow.getFlowNodes().size()));
        RpcContext.getContext().setAttachment(getFlowName(), String.valueOf(flow.getName()));
        for (TransactionNode node : flow.getFlowNodes()) {
            RpcContext.getContext().setAttachment(getNodeKey(), serializeToString(node));
        }
